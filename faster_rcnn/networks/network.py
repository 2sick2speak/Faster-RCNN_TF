...
class Network(object):
    ...
    def load(self, data_path, session, saver, ignore_missing=False):
        if data_path.endswith('.ckpt'):
            ...
            data_dict = np.load(data_path, fix_imports=True, encoding='bytes').item()
            for key in data_dict:
                with tf.variable_scope(key.decode(), reuse=True):
                    for subkey in data_dict[key]:
                        try:
                            var = tf.get_variable(subkey.decode())
                            session.run(var.assign(data_dict[key][subkey]))
                            print("assign pretrain model "+subkey.decode()+ " to "+key.decode())
                        except ValueError:
                            print("ignore "+key.decode())
    ...
    def anchor_target_layer(self, input, _feat_stride, anchor_scales, ratios, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            rpn_labels,rpn_bbox_targets,rpn_bbox_inside_weights,rpn_bbox_outside_weights = tf.py_func(
                anchor_target_layer_py,[input[0],input[1],input[2],input[3], _feat_stride, anchor_scales, ratios],[tf.float32,tf.float32,tf.float32,tf.float32])
